...
# Updated with HARD NEGATIVES to force context-based learning
# NOTE: Significantly increased git SHA samples to suppress false positives
NEGATIVE_DISTRIBUTION = {
    # Standard non-secrets - hex tokens
    "git_sha": 0.08,          # Pure hex (40 chars) - generic context
    "checksum": 0.06,         # Hex hashes
    "uuid": 0.04,
    "data": 0.03,
    ...
    "ambiguous_key": 0.05,
    # --- HARD NEGATIVES (force context-based learning) ---
    "doc_example": 0.09,      # Documentation placeholders like AKIAIOSFODNN7EXAMPLE
    "revoked_token": 0.07,    # Valid format tokens in revocation contexts
    "test_fixture": 0.09,     # Test tokens like AKIATESTKEY123456789
    "encoded_config": 0.05,   # Base64 JSON configs, not secrets
    "high_entropy_safe": 0.08,  # Hashes, session IDs, cache keys
    # --- GIT SHA EXPLICIT CONTEXT (suppress false positives) ---
    "git_sha_explicit": 0.15,  # Git SHAs with commit_sha, git_hash var names
    "content_hash_explicit": 0.10,  # Content hashes with checksum, digest var names
    # --- CERTIFICATES (public, not secrets) ---
    "certificate": 0.03,  # X.509 certificates
}
...
def _get_var_name(positive: bool) -> str:
    """
    ...
    # For negative samples, sometimes use a secret-looking name
...
def generate_training_record(positive: bool) -> Dict[str, Any]:
    ...
    if positive:
        ...
        # 3. For git_sha_explicit and content_hash_explicit, use specialized generators
        #    that return (token, var_name, context_type) tuples
    ...
    if specialized_var_name:
        ...
        var_name = _get_var_name(positive)
