...
class TestTokenFormatShortcuts:
    ...
    def test_prefix_distribution_balanced(self):
        ...
        # Negative samples should have at least 15% with prefixes
        # (Lowered from 25% due to increased SHA-like/hex negatives for Stage B precision
        #  and hash_secret_varname for Stage B generalization)
        assert neg_prefix_ratio >= 0.15, (
            f"Too few negative samples with prefixes: {neg_prefix_ratio:.1%}. "
            f"Expected >= 15%. Found {negative_has_prefix}/{total_negative}."
        )
...
class TestFeatureVector:
    ...
    def test_feature_count_is_65(self):
        """Verify we have exactly 65 features (63 original + 2 env-loading features)."""
        names = FeatureVector.get_feature_names()
        assert len(names) == 65, f"Expected 65 features, got {len(names)}"

        vec = FeatureVector()
        arr = vec.to_array()
        assert len(arr) == 65, f"Expected 65 values, got {len(arr)}"
    ...
    def test_feature_extraction_works(self):
        ...
        for record in records:
            ...
            assert len(arr) == 65, f"Expected 65 features, got {len(arr)}"
